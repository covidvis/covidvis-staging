...
vegaEmbed("#jhu_us_cases", jhu_us_cases, embedOpt).then(function(chart) {
    chart.view.addSignalListener('click', makePopulateInfoPageSpaceHandler('jhu_us_cases'));
});
vegaEmbed("#jhu_us_deaths", jhu_us_deaths, embedOpt).then(function(chart) {
    chart.view.addSignalListener('click', makePopulateInfoPageSpaceHandler('jhu_us_deaths'));
});
